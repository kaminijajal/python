...
def add_data(request):
    try:
        ...
        if not ser.is_valid():
            return Response({"error":ser.errors,"message":"Somthing wrong.."},status=status.HTTP_400_BAD_REQUEST)
        else:
            ser.save()
...
def get_data_id(request,id):
    try:
        ...
        return Response({"message":"Something worng..."},status=status.HTTP_400_BAD_REQUEST)
...
def delete_data(request,id):
    try:
        emp = Employee.objects.get(pk=id)
        emp.delete()
        return Response({"meaage":"message was deleted..."})
    except Employee.DoesNotExist:
        return Response(
            {"message": "Employee not found", "errors": f"No employee found with id {id}"},
            status=status.HTTP_404_NOT_FOUND
